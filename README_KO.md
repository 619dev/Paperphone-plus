...
[![Version](https://img.shields.io/badge/버전-2.4.6-orange)](client/package.json)
...
| 🗝️ 제로 지식 암호화 | 암호화된 대화의 경우 서버는 암호문을 저장하지만, 계정·연락처/그룹·라우팅·푸시에 필요한 메타데이터는 처리합니다. 식별 개인 키와 Sender Key는 로컬에만 저장되며, Web은 AES-GCM으로 래핑한 IndexedDB를, Android·iOS·Windows·macOS는 OS 보안 저장소를 사용합니다 |
| 🎭 텍스트 모양과 추가 암호화 | 프로필 > 메시지 개인정보에서 이 기기의 모든 채팅에 추가 비밀번호를 설정하고 8가지 텍스트 모양으로 본문을 표시합니다. 수동 잠금과 자동 잠금 지원 |
...
| 📱 세션 유지 | 30분 Access Token과 자동 연장되는 90일 기기 Refresh Session. 네트워크/IP/VPN/프록시 변경 후에도 비밀번호 없이 자동 복구 |
| 📨 신뢰성 있는 메시지 동기화 | 양방향 하트비트, 반개방 연결 감지, 영구 보낼 편지함, 멱등 메시지 ID 및 서버 시퀀스 커서 기반 누락 복구 |
| 📴 오프라인 접근 | 계정별로 연락처, 그룹, 대화당 최대 2,000개 메시지, 모먼트, 타임라인 및 미디어를 캐시하며 오프라인 전송은 자동 재시도 |
| 🔎 Unicode 친구 검색 | IME 조합 보호, NFC 정규화 및 UTF-8 쿼리 인코딩으로 중국어 사용자 이름과 별명을 안정적으로 검색 |
...
PaperPhonePlus는 로컬 계정 상태, 실시간 연결 상태, 메시지 동기화 상태를 분리하여 처리합니다. WebSocket이 열려 있어도 서버의 `auth_ok`를 받기 전에는 사용 가능한 연결로 간주하지 않습니다. 양방향 `ping/pong`으로 VPN/IP 변경, Wi-Fi/모바일 전환, 앱 일시 중단으로 생기는 반개방 연결을 감지합니다.

- Access Token은 30분 동안 유효합니다. 기기 Refresh Token은 90일 동안 유효하며 활발히 사용하는 동안 연장되어 비밀번호 재입력 없이 갱신됩니다.
- 이전 버전에서 이미 로그인한 기기는 기존 Token이 유효한 동안 자동 업그레이드됩니다. 이미 만료된 경우에만 마지막으로 한 번 직접 로그인해야 합니다.
- 모든 발신 메시지에는 안정적인 `client_msg_id`가 있습니다. 서버 ACK를 받지 못한 메시지는 영구 로컬 보낼 편지함에 남아 같은 ID로 재전송되며, 서버 고유 제약 조건이 중복 저장을 방지합니다.
- 저장된 메시지에는 단조 증가하는 `server_seq`가 있습니다. 인증, 재연결, 포그라운드 복귀 후 커서 기반으로 누락된 메시지를 동기화합니다.
- 명시적 로그아웃과 기기 취소는 장기 서버 세션을 무효화합니다. 일반적인 전송 오류와 IP 변경은 로그인 상태를 유지합니다.

> [!IMPORTANT]
> 업그레이드 시 server를 먼저 배포하고 client를 나중에 배포하세요. server는 신뢰성 DB 마이그레이션을 자동 적용·검증하며 필수 열이 없으면 시작하지 않습니다. 프로덕션 업그레이드 전에 MySQL을 백업하세요.
...
암호화 레이어
  ...
  로컬 키 보호: Web은 AES-GCM으로 래핑한 IndexedDB, Android/iOS/Windows/macOS는 OS 보안 저장소 사용
  식별 개인 키와 Sender Key는 로컬에 저장되며 서버로 전송되지 않습니다
```
...
**프로필 > 메시지 개인정보**에서 이 기기의 모든 채팅에 추가 비밀번호를 사용할 수 있습니다. 전송 전에 본문을 추가로 암호화해 텍스트 모양으로 변환한 뒤, 기존 개인 채팅 E2EE(X25519 / ML-KEM-768) 또는 그룹 Sender Key 암호화로 다시 보호합니다. 즉 기존 E2EE 위의 두 번째 본문 보호 계층입니다. 외관은 **불교식 텍스트, 무작위 중국어, 주역 기호, 한글, 이집트 상형문자, 설형문자, 핵심 가치 텍스트, 영숫자** 중에서 선택합니다.

- 비밀번호는 업로드되거나 자동 동기화되지 않습니다. 개인 채팅 양쪽 또는 그룹의 모든 멤버가 같은 추가 비밀번호를 각자 설정해야 합니다.
- 텍스트 모양은 서로 같을 필요가 없습니다. 각 메시지에 모양 식별자가 포함되므로 수신 측에서 발신자가 선택한 모양을 자동으로 인식하고 복원합니다. 예를 들어 한쪽은 불교식 텍스트, 다른 쪽은 한글 모양을 사용해도 추가 비밀번호만 같으면 양쪽 모두 정상적으로 복호화할 수 있습니다. 각자의 설정은 자신이 보내는 암호문의 모양만 결정합니다.
- 비밀번호가 다르면 E2EE와 메시지 전송은 정상 작동하지만 텍스트 모양 암호문만 보이고 원문은 읽을 수 없습니다.
- 비밀번호는 8자 이상이며 잠금 해제 중에만 메모리에 유지됩니다. 로컬에는 salt와 비밀번호 검증 정보만 저장됩니다.
- 즉시 잠그거나 포그라운드를 벗어난 뒤 **5 / 15 / 30 / 60분** 후 자동으로 잠글 수 있습니다. 잠김 상태에서는 스타일된 암호문만 표시됩니다.
- 추가 암호화를 끄려면 현재 잠금 해제되어 있어도 올바른 비밀번호를 다시 입력해야 합니다.
- 텍스트 모양은 기존 E2EE 위의 추가 보험이며 **E2EE를 대체·우회·다운그레이드하지 않습니다**.
