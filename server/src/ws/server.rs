...
async fn flush_offline_messages(state: &Arc<AppState>, user_id: &str, tx: &mpsc::UnboundedSender<String>) {
    // ── Step 1: Purge expired messages from DB before flushing ──────────
    // Delete expired private messages for this user
    sqlx::query(
        "DELETE m FROM messages m
         JOIN friends f ON (f.user_id = m.from_id AND f.friend_id = m.to_id)
         WHERE m.type = 'private'
           AND (m.to_id = ? OR m.from_id = ?)
           AND f.auto_delete > 0
           AND m.created_at < DATE_SUB(NOW(), INTERVAL f.auto_delete SECOND)"
    ).bind(user_id).bind(user_id).execute(&state.db).await.ok();

    // Delete expired group messages in groups this user belongs to
    sqlx::query(
        "DELETE m FROM messages m
    ...
    // ── Step 2: Flush remaining valid offline private messages ──────────
